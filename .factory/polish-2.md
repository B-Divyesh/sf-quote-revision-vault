...
- Fresh clone: `/tmp/qrv-polish-2-clean`, `npm ci` completed with 0 vulnerabilities. Every one of the 13 commands named in `.factory/claims.json` passed separately, followed by `npm test` (4 registry tests + 46 Playwright tests), `npm run typecheck`, and `npm run build`.
