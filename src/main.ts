  ...
  page(`<main id="main">
    <section class="hero" aria-labelledby="landing-title">
      <div class="hero-copy">
        ...
        <ul class="facts"><li>Quote data stays here until you share it</li><li>Works offline after your first visit</li><li>Free for one quote · $29 once for multiple quotes</li></ul>
      </div>
      <div class="hero-art">
...
function bindHistory(quote: Quote) {
  ...
  document.querySelectorAll<HTMLElement>('[data-revoke-share]').forEach((button) => button.addEventListener('click', async () => {
    ...
    try { await revokeReviewLink(share.id, share.ownerKey); share.revokedAt = new Date().toISOString(); await saveQuote(quote); renderVault(); status('The review link is blocked on every device.'); }
...
async function createShare(quote: Quote) {
  ...
  try { await registerReviewLink(shareId, expiresAt, ownerKey); }
  ...
  const packet: SharePacket = { version: 2, shareId, quoteId: quote.id, revisionId, revisionNumber: revision.number, expiresAt, snapshot: revision.snapshot };
  const url = `${location.origin}/ack#packet=${encode(packet)}`;
  output.innerHTML = `<p class="share-output"><strong>Review link created.</strong><br><span>${escapeHtml(url)}</span></p><button class="button secondary small" id="copy-share" type="button">Copy review link</button>`;
...
async function acknowledgment() {
  ...
  try { state = await getReviewLinkState(packet.shareId); }
