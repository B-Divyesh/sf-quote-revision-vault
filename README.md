...
Quotes stay in IndexedDB until you export a vault file or copy a review link. The app works offline after the first visit. The free tier creates one quote with unlimited revisions. A $29 one-time Studio Pass permits more than one quote.
...
The customer can return an acknowledgment code. This records review only and is not a legal signature. The creating browser can block its copy of a link. Every link checks its embedded expiry when opened.

## Privacy and payment

There are no analytics, ads, remote fonts, or runtime CDNs. License verification sends only the pasted license token to Sociobot. Checkout uses the Sociobot billing API; no payment provider is embedded here.
